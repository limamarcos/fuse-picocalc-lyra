 ...
 */
  ...
  if( snd_pcm_open( &pcm_handle, pcm_name , stream, SND_PCM_NONBLOCK ) < 0 ) {
    ...
    /* we try a last one: plughw:0,0 but what a weired ALSA conf.... */
      if( snd_pcm_open( &pcm_handle, "plughw:0,0", stream, SND_PCM_NONBLOCK ) < 0 ) {
...
sound_lowlevel_frame( libspectrum_signed_word *data, int len )
{
  ...
  while( len > 0 ) {
    ...
    if( ret == len ) {
      ...
      /* Buffer full in non-blocking mode: drop remainder to avoid emulator stall */
      break;
    } else if( ret < 0 ) {
      fuse_log( "SOUND", "snd_pcm_writei error: %d (%s), recovering", ret, snd_strerror( ret ) );
      if( ++retry_count > 2 ) {
        fuse_log( "SOUND", "[WARN] snd_pcm_writei retry limit reached, dropping %d frames", len );
        break;
      }
      int rec_err = snd_pcm_recover( pcm_handle, ret, 1 );
      if( rec_err < 0 ) {
        fuse_log( "SOUND", "[ERROR] snd_pcm_recover failed: %d (%s)", rec_err, snd_strerror( rec_err ) );
        break;
      }
    }
  }
